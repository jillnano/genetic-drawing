...
class GeneticDrawing:
    def __init__(self, img_path, seed=0, brushesRange=[[0.1, 0.3], [0.3, 0.7]]):
        ...
        # img_grey = cv2.cvtColor(self.original_img,cv2.COLOR_BGR2GRAY)
        self.img_grads = self._imgGradient(self.original_img)
        self.myDNA = None
        self.seed = seed
        ...
        self.imgBuffer = [np.zeros(self.original_img.shape, np.uint8)]
    ...
    def generate(self, stages=10, generations=100, brushstrokesCount=10, show_progress_imgs=True):
        for s in range(stages):
            ...
            self.myDNA = DNA(self.original_img.shape, 
                             self.img_grads, 
            ...
            for g in range(generations):
                self.myDNA.evolveDNASeq(self.original_img, self.seed + time.time() + g)
                clear_output(wait=True)
                print("Stage ", s+1, ". Generation ", g+1, "/", generations)
                if (s+1) % 10 == 0 and g+1 == generations:
                    fn = 'output/sp_%s.jpg'%(s+1)
                    print(fn)
                    cv2.imwrite(fn, self.myDNA.get_cached_image())
...
def util_sample_from_img(img):
    ...
    pos = np.indices(dimensions=img_gray.shape)
    pos = pos.reshape(2, pos.shape[1]*pos.shape[2])
    img_flat = np.clip(img_gray.flatten() / img_gray.flatten().sum(), 0.0, 1.0)
...
class DNA:
    ...
    def initRandom(self, target_image, count, seed):
        ...
        for i in range(count):
            ...
            localMag = np.mean(self.imgMag[posY][posX])
            localAngle = np.mean(self.imgAngles[posY][posX]) + 90 #perpendicular to the dir
    ...
    def __evolveDNA(self, index, inImg, seed):
        ...
        for changeIndex in changeIndices:
            if changeIndex == 0:# if color
                ...
                localMag = np.mean(self.imgMag[int(child[1])][int(child[2])])
                localAngle = np.mean(self.imgAngles[int(child[1])][int(child[2])]) + 90 #perpendicular
...
if __name__ == '__main__':
    ret = GeneticDrawing('WechatIMG2339.jpeg').generate(100, show_progress_imgs = False)
    cv2.imwrite('sp_out.jpg', ret)